...
if [ -f "./build/lib.linux-x86_64-2.7/bson/_cbson.so" ]; then
#if [ $? -eq 0 ]; then
    ...
    # scripts.
    if [ -f "./build/temp.linux-x86_64-2.7/pymongo/_cmessagemodule.o" ]; then
		md5sum ./build/lib.linux-x86_64-2.7/bson/_cbson.so ./build/temp.linux-x86_64-2.7/pymongo/_cmessagemodule.o 
	else
		echo FAIL
	fi
else
