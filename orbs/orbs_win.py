...
#
def setup():
    #r = commands.getoutput('bash config/setup.sh')
    r = subprocess.check_output('bash config/setup.sh', shell=True)
...
def execute(log):
    ...
    return subprocess.call('bash config/execute.sh ' + criterion + " " + log, shell=True)
    ...
    while line >= 0 and line < len(lines):
        ...
        if rc != "FAIL":
            ...
            if projected == original:
                print "projected and orginal same:"
                print projected
