...
#
def setup():
    r = subprocess.getoutput('bash config/setup.sh')
...
def compile(log):
    ...
    count_compiles += 1
    r = subprocess.getoutput('bash config/compile.sh ' + log)
    debug.write("C " + log + " " + r + "\n")
    return r
...
def execute(log):
    ...
    count_executes += 1
    return subprocess.getoutput('bash config/execute.sh ' + criterion + " " + log)
